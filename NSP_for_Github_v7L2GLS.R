...
# Date           : 2022-11-22 12:08:38
# Last Revision  : 2022-11-23 09:13:39
# Last Author    : Jie Li
# File Path      : /nsp-piotr/NSP_for_Github_v7L2GLS.R
# Description    :
#
#
#
...
check_interval_array <- function(ind, ads.array, thresh) {
    ...
    result <- gls(y ~ x - 1, correlation = corAR1(0.5))
    linf <- max(abs(residuals(result)))
...
lp_selfnorm <- function(ind, ads.array, selfnorm.array) {
    ...
    x <- f.con.rhs.core[, -1]
    # max_shifts <- max(ads.array$shifts)
    result <- gls(y ~ x - 1, correlation = corAR1(0.5, max_shifts))
    linf <- max(abs(residuals(result)))
    linf
}

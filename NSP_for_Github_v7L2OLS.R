...
# Last Revision  : 2022-11-23 09:58:32
# Last Author    : Jie Li
# File Path      : /nsp-piotr/NSP_for_Github_v7L2OLS.R
# Description    :
#
#
#
...
check_interval_array <- function(ind, ads.array, thresh) {
    ...
    result <- lm(y_init ~ x_init - 1, weight = freq)
    residual <- f.con.rhs.core[, 1] - f.con.rhs.core[, -1] %*% coef(result)
    linf <- max(abs(residual))
...
lp_selfnorm <- function(ind, ads.array, selfnorm.array) {
    ...
    for (i in 1:scales) {
        shift <- ads.array$shifts[i]
        f.con.rhs.current <- ads.array$res[1:(n - shift), , i] / selfnorm.array[1:(n - shift), i]
